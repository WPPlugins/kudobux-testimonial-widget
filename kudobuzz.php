	...
	if ( isset( $rs_time ) && $rs_time != null && $rs_time != false ){
		...
		if($now >= $rs_time){
			if (isset($kd_uid) && $kd_uid !== FALSE && !empty($kd_uid)) {
				...
				$response = run_curl($url, 'GET', null);
				$response_object = json_decode($response);

				if( isset($response_object->account) ){

					if(in_array($response_object->account->payment_plan->plan->id, array(3,4,7,8,10,12,14,16))){

						$url = API_URL.'widget/reviews/'. $kd_uid;
						$response = run_curl($url, 'GET', null);
						$response_object = json_decode($response);
						if( isset($response_object->site_reviews) ){

							$total = 0;
							foreach ($response_object->site_reviews->rating_breakdown as $key => $value){
								$total = $total + $value;
							}
							$avg_rating = floatval($response_object->site_reviews->avg_rating);
							update_option('kudobuzz_ratingValue', $avg_rating);
...
function post_create_account_action() {
	...
	try{
		$response = run_curl($url, 'POST', $data);
		$response_object = json_decode($response);
		if(isset($response_object->user)){
			update_option( 'kudobuzz_uid', $response_object->uid );
			update_option( 'kudobuzz_token', $response_object->token );
			update_option( 'kudobuzz_user_id', $response_object->user->id );
			update_option( "kudobuzz_email", $response_object->user->email );
			echo 1;
			wp_die();
		}else{
			echo 0;
